...
void motorInit() {
  ...
  analogWriteFrequency(PWM1_PIN, MOTOR_PWM_FREQ_HZ);
  analogWriteFrequency(PWM2_PIN, MOTOR_PWM_FREQ_HZ);
  analogWriteResolution(PWM1_PIN, 8);
  analogWriteResolution(PWM2_PIN, 8);
...
void setMotors(uint8_t directionByte, uint8_t leftSpeed, uint8_t rightSpeed) {
  ...
  writePwmDuty(PWM1_PIN, rightSpeed);
  writePwmDuty(PWM2_PIN, leftSpeed);
  digitalWrite(STCP_PIN, LOW);
...
void handleGpsNorthTest() {
  ...
  // Lost fix → stop and wait
  if (!hasFreshGpsFix()) {
    if (gpsNorthState != NORTH_WAIT_FOR_FIX) {
      Serial.println("[North] GPS fix lost. Stopping until fix returns.");
      moveStop();
      gpsNorthState = NORTH_WAIT_FOR_FIX;
    }
    return;
  ...
  switch (gpsNorthState) {
    ...
    case NORTH_COMPARE: {
      ...
      Serial.printf("[North] lat %.6f → %.6f (gain %.6f) | course %.1f° | sats %lu\n",
                    gpsNorthStartLat, currentLat, latGain, course,
                    gps.satellites.isValid() ? gps.satellites.value() : 0);
...
void printHelpMenu() {
  ...
  Serial.println("=========================================");
  Serial.println("External LEDs: slow = waiting GPS, fast = GPS data seen");
